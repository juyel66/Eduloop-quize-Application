      ...
      <div className="space-y-3 mb-6 grid grid-cols-2 gap-4">
        {options.map((option) => (
          <button
            ...
            className={`w-full text-left h-12 p-3 rounded-lg border transition ${
              selected === option && result === null ? "border-blue-500 bg-blue-50" : "border-gray-300 bg-white"
            } ${result !== null && option === correctAnswer ? "border-green-500 bg-green-900" : ""} ${result === false && selected === option && option !== correctAnswer ? "border-red-500 bg-red-50" : ""}
            `}
          >
            {option}
          </button>
