...
export default function ArrType_31({ hint }: { hint: string }) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-8 px-6 py-8">
        {problemsJSON.map((p, idx) => (
          <div key={p.id} className="flex flex-col items-start w-full">
            {/* Question Card */}
            <div className="rounded-2xl bg-[#FFEDD5] p-6 flex items-center shadow-md w-full mb-4">
            ...
            <div className="flex flex-col text-green-600 w-full px-2">
              ...
              <input
                type="text"
                className="flex-1 p-1 text-sm border-b border-dotted border-gray-800 outline-none text-gray-700 bg-transparent"
